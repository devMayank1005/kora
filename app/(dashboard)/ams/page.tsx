    ...
    return (
      <div className="flex h-80 items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs font-medium text-slate-400">Loading AMS support console…</span>
        </div>
      </div>
    );
  }
    ...
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200/60 pb-5 dark:border-slate-800">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Managed service billing, retainer burn & ticket governance
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200/80 bg-white p-3.5 shadow-2xs dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center gap-2">
          <button
            onClick={prevMonth}
            className="rounded-xl border border-slate-200 p-2 text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 transition-colors"
            title="Previous month"
          >
            <ChevronLeft className="h-4 w-4" />
          </button>

          <span className="px-4 text-xs font-bold text-slate-900 dark:text-white min-w-[150px] text-center font-mono">
            {monthName}
          </span>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="rounded-xl bg-slate-100 px-3.5 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-200 transition-colors"
          >
            Current Month
          </button>
        </div>
          ...
          {billing && (
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-2xs dark:border-slate-800 dark:bg-slate-900">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Consumed Hours</span>
              ...
              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-2xs dark:border-slate-800 dark:bg-slate-900">
                ...
                <div className="mt-2 text-2xl font-extrabold text-[#0891b2] font-mono">
                  {billing.billableDays.toFixed(2)}d
                </div>
              </div>

              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-2xs dark:border-slate-800 dark:bg-slate-900">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Estimated Burn</span>
                ...
                {filteredEntries.map(e => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="mt-1 text-xs text-slate-500 truncate">{e.description || '—'}</p>
                      <div className="mt-2.5 flex items-center gap-1.5">
                        <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-[10px] font-mono font-bold text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                          {e.hours}h
                        </span>
                ...
                <div className="space-y-6">
                  <div>
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-bold text-blue-700 dark:bg-blue-950/40 dark:text-blue-400">
                  ...
                  <div className="grid grid-cols-2 gap-3.5 sm:grid-cols-4">
                    ...
                    <div className="rounded-xl border border-slate-100 bg-slate-50/70 p-3.5 dark:border-slate-800 dark:bg-slate-800/40">
                      ...
                      <div className="mt-1.5 text-xs font-bold text-slate-900 dark:text-white truncate">
                    ...
                    <div className="rounded-xl border border-slate-100 bg-slate-50/70 p-3.5 dark:border-slate-800 dark:bg-slate-800/40">
                      ...
                      <div className="mt-1.5 text-xs font-bold text-slate-900 dark:text-white truncate font-mono">
                    ...
                    <div className="rounded-xl border border-slate-100 bg-slate-50/70 p-3.5 dark:border-slate-800 dark:bg-slate-800/40">
                      ...
                      <div className="mt-1.5 text-xs font-bold text-[#0891b2] font-mono">
                        {activeEntry.hours}h
                      </div>
                    </div>
                  </div>

                  {/* Solution & Discussion Notes */}
                  <div className="rounded-xl border border-cyan-100 bg-cyan-50/40 p-4 dark:border-slate-800 dark:bg-slate-800/30">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-[#0891b2]">
                      💡 Solution & Resolution Notes
                    </h3>
                    <p className="mt-1.5 text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
                      {activeEntry.solutionDiscussed || 'No resolution notes recorded yet.'}
                    </p>
                  </div>
                </div>
              ) : (
